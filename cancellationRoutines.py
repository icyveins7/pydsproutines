...
if __name__ == "__main__":
    ...
    _, sig = addSigToNoise(
        length,
        0,
        syms1 + syms2 * np.exp(1j * np.random.rand() * 2 * np.pi),
        snr_inband_linear=10.0,
    )
    ...
    acc = (
        1 / (length / factor) ** 2
    )  # 1.0 factor is when it seems to start to happen, 10.0 and it's essentially unrecoverable i.e. cancellation doesn't work
    ...
    win, ax = pgPlotAmpTime(
        [sigacc, cancelled], [1, 1], ["orig", "cancelled"], ["w", "r"]
    )
    ...
    compensated = syms1 * np.exp(
        1j * 2 * np.pi * 0.5 * estacc * np.arange(syms1.size) ** 2
    )
