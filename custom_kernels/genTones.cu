...
// Note: we revert to calculating all complex numbers in complex floats, or else the speed is actually slower;
// in fact, the kernel duration is pretty long even in floats, but we benefit from a single kernel call, as opposed to
// multiple fft, ifft, mul calls when using the gpu czt (which incurs significant CPU overhead)
extern "C" __global__
void dotTonesScaling_32f(
	...
	complex<float> *out)
{
	const int SHARED_MEM_COLS = 65; // we only put this as +1 to alleviate shared mem bank conflicts
	const int SHARED_MEM_ROWS = 64;
	...
	complex<float> *s_ws = (complex<float>*)s; // (SHARED_MEM_ROWS*SHARED_MEM_COLS) complex floats
	...
	// calculate the value for the first frequency
	sincospi(2 * f0 * (double)i, &im, &re);
	complex<float> outstack(re, im); // we keep a copy on stack
	
	// now calculate the complex number to scale by
	sincospi(2 * fstep * (double)i, &im, &re);
	// complex<double> alpha(re, im);
	complex<float> alpha(re, im);
	...
	for (int fidx = 0; fidx < numFreqs; fidx++)
	{
		...
		if (fidx % 64 == 0 & fidx != 0)
		{
			...
			for (int k = 0; k < kMax; k++) // note that not all columns may be used (if we're the last block)
			{
				...
				accumulator += s_ws[threadIdx.x * SHARED_MEM_COLS + k];
			}
		...
		{
			if (i < len) // only if we are within the src length
			{
			...
			{
				outstack = 0; // otherwise we can just write zeroes to the shared mem column
			}
		}

		// after we have done the multiply of the tone, we save it in the appropriate spot in the workspace on shared mem
		// s_ws[SHARED_MEM_COLS * s_row + threadIdx.x] = complex<float>(outstack.real(), outstack.imag()); // cast to floats just like before
		s_ws[SHARED_MEM_COLS * s_row + threadIdx.x] = outstack; // cast to floats just like before
	}
	...
	if (threadIdx.x < numFreqs - batch * SHARED_MEM_ROWS)
	{
		// then we iterate across the columns
		accumulator = 0.0f;
