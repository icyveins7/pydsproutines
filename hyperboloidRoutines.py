...
class Hyperboloid:
        ...
        for i in np.arange(tc.shape[1]):
            ...
            # if True:
                ...
                if thetas.size == 2:
                    ...
                    pctr += 1
                # if thetas.size == 1:
                #     print("WARNING: only found 1 real root for v=%f" % (v[i]))
                #     print(roots)
    ...
    def intersectOblateSpheroid(
        ...
        # if autoGeneratedV:
        #     # Take the smallest v with real roots and extend down a bit
        #     vpow = np.log(vstep)
        #     vext = v[0] - np.exp(np.linspace(vpow+1,vpow))
        #     print(vext)
        #     print(v)
...
if __name__ == "__main__":
    ...
    timer.start()
    sathyp, sve = hsat.intersectOblateSpheroid() # v=np.logspace(-10,0,1000)+4.2) # v=np.arange(4.2,4.3,1e-6))
    timer.end()
