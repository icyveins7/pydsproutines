...
class Hyperboloid:
    def __init__(
        ...
        foci = np.array([[0, 0, -self.focusZ], [0, 0, self.focusZ]])
        self.foci = np.zeros((3, 2))
        self.foci[0, :], self.foci[1, :], self.foci[2, :] = self.transform(
            foci[:, 0], foci[:, 1], foci[:, 2]
        )
    ...
    def transform(self, X, Y, Z):
        ...
        vecs = np.vstack((X.reshape(-1), Y.reshape(-1), Z.reshape(-1)))
    ...
    # Other methods
    def visualize(
        self, v=np.arange(0, 2, 0.1), ax=None, bothSheets=False, useSurf=False
    ):
        if ax is None:
            fig = plt.figure()
        ...
        # Ensure equal ratios
        self.visxlim = np.ptp(
            np.hstack((Xp.reshape(-1), Xm.reshape(-1)))
        )  # We keep these around for future reference
        self.visylim = np.ptp(
            np.hstack((Yp.reshape(-1), Ym.reshape(-1)))
        )  # in order to plot other objects around to scale together
        self.viszlim = np.ptp(np.hstack((Zp.reshape(-1), Zm.reshape(-1))))
        ax.set_box_aspect((self.visxlim, self.visylim, self.viszlim))
        ...
        if useSurf:
            ...
            if bothSheets:
                ax.plot_wireframe(
                    Xp, Yp, Zp, color="k", linestyle="--"
                )  # In order to see the foci, use wireframe rather than surface
            ax.plot_wireframe(Xm, Ym, Zm, color="k", linestyle="-")

        # Plot foci
        ax.scatter3D(self.foci[0, :], self.foci[1, :], self.foci[2, :], c="r")
        ax.set_title(
            "Foci: (%.3f, %.3f, %.3f), (%.3f, %.3f, %.3f)"
    ...
    def _intersectXYsheet(self, v, sign):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            theta1 = np.arcsin(
                b
            )  # If there are NaNs here, it means that the particular v does not have a solution!
        ...
        vect[0, :], vect[1, :], vect[2, :] = self.transform(
            vec[0, :], vec[1, :], vec[2, :]
        )
    ...
    def intersectXY(self, v=np.arange(0, 2, 0.01), onlyReturnOneSheet=False):
        msheet = self._intersectXYsheet(
            v, -1
        )  # This is the main sheet which corresponds correctly to range diff
    ...
    def _generateIntersectOblateSpheroidCoefficients(self, v, omega, lmbda):
        ...
        l_2 = (
            self.Rot[0, 2] * mc_coshv + self.mu[0]
        )  # For now, let's just work on the correct sheet
        # Y components
        m_0 = self.Rot[1, 0] * a_sinhv
        m_1 = self.Rot[1, 1] * a_sinhv
        m_2 = (
            self.Rot[1, 2] * mc_coshv + self.mu[1]
        )  # For now, let's just work on the correct sheet
        # Z components
        n_0 = self.Rot[2, 0] * a_sinhv
        n_1 = self.Rot[2, 1] * a_sinhv
        n_2 = (
            self.Rot[2, 2] * mc_coshv + self.mu[2]
        )  # For now, let's just work on the correct sheet
    ...
    def intersectOblateSpheroid(
        ...
        thetarealminus, veminus, thetarealplus, veplus = (
            self._intersectOblateSpheroidLoop(tc, v)
        )
        ...
        if refineMiddle:
            ...
            vext = np.linspace(
                veplus[0] - vspace, veplus[0], numPts // 2, endpoint=False
            )  # Split within the nearest step, with fewer steps required

            # Do it all again
            tcext = self._generateIntersectOblateSpheroidCoefficients(
                vext, omega, lmbda
            )
            ethetarealminus, eveminus, ethetarealplus, eveplus = (
                self._intersectOblateSpheroidLoop(tcext, vext)
            )

            # Stitch lists
            nthetareals = np.hstack(
                (thetarealminus, ethetarealminus, ethetarealplus, thetarealplus)
            )
...
if __name__ == "__main__":
    ...
    hp = Hyperboloid.fromFoci(np.array([-1, 0, 0]), np.array([1, 0, 0]), -1.0)
    ax, fig = hp.visualize()
    ...
    ha = Hyperboloid.fromFoci(np.array([-1, -1, -1]), np.array([1, 1, 1]), 1.0)
    ax, fig = ha.visualize()
    # Perform intersection
    ...
    hat = Hyperboloid.fromFoci(
        10 + np.array([-1, -1, -1]), 10 + np.array([1, 1, 1]), 1.0
    )
    ...
    hsp = Hyperboloid.fromFoci(np.array([-10, 1, 0]), np.array([-10, -1, 0]), 0.5)
    ...
    timer.start()
    sathyp, sve = hsat.intersectOblateSpheroid(
        numPts=1000
    )  # We have now adapted the algo to generate points better at the centre
    ...
    class TestHyperboloids(unittest.TestCase):
        def test_zero_plane(self):
            ...
            np.testing.assert_allclose(hp.foci[:, 0], [-1, 0, 0], atol=1e-7)
            np.testing.assert_allclose(hp.foci[:, 1], [1, 0, 0], atol=1e-7)
            # Test the sheet correctness
            vm = hp.intersectXY(onlyReturnOneSheet=True)
            np.testing.assert_allclose(
                np.linalg.norm(vm - hp.foci[:, 1].reshape((-1, 1)), axis=0)
                - np.linalg.norm(vm - hp.foci[:, 0].reshape((-1, 1)), axis=0),
                np.zeros(vm.shape[1]) - 1.0,
            )

        def test_zero_orient(self):
            # Test the foci locations
            np.testing.assert_allclose(ha.foci[:, 0], [-1, -1, -1], atol=1e-7)
            np.testing.assert_allclose(ha.foci[:, 1], [1, 1, 1], atol=1e-7)
            # Test the sheet correctness
            vm = ha.intersectXY(onlyReturnOneSheet=True)
            np.testing.assert_allclose(
                np.linalg.norm(vm - ha.foci[:, 1].reshape((-1, 1)), axis=0)
                - np.linalg.norm(vm - ha.foci[:, 0].reshape((-1, 1)), axis=0),
                np.zeros(vm.shape[1]) + 1.0,
            )

        def test_trans_orient(self):
            # Test the foci locations
            np.testing.assert_allclose(hat.foci[:, 0], [9, 9, 9], atol=1e-7)
            np.testing.assert_allclose(hat.foci[:, 1], [11, 11, 11], atol=1e-7)
            # Test the sheet correctness
            vm = hat.intersectXY(onlyReturnOneSheet=True)
            np.testing.assert_allclose(
                np.linalg.norm(vm - hat.foci[:, 1].reshape((-1, 1)), axis=0)
                - np.linalg.norm(vm - hat.foci[:, 0].reshape((-1, 1)), axis=0),
                np.zeros(vm.shape[1]) + 1.0,
            )

        def test_random_hyperboloid(self):
            s = np.random.rand(3, 2)
            rangediff = np.linalg.norm(s[:, 1] - s[:, 0]) * (
                np.random.rand() * 2 - 1
            )  # Randomly choose either one
            hr = Hyperboloid.fromFoci(s[:, 0], s[:, 1], rangediff)
            # Generate the correct sheet
            vm = hr.intersectXY(onlyReturnOneSheet=True)
            np.testing.assert_allclose(
                np.linalg.norm(vm - hr.foci[:, 1].reshape((-1, 1)), axis=0)
                - np.linalg.norm(vm - hr.foci[:, 0].reshape((-1, 1)), axis=0),
