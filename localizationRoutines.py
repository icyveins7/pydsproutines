...
def calculateRangeRate(
    ...
    tx_xdot_p = np.dot(
        dirvec, tx_xdot
    )  # This ordering of dirvec first lets us have multiple rows of dirvecs dotted with a single 1D xdot array
...
def hyperboloidLineIntersectCostFunc(delta, x0, s1, s2, rangediff, g):
    return (
        rangeOfArrival(x0 + g * delta, s2)
        - rangeOfArrival(x0 + g * delta, s1)
        - rangediff
    ) ** 2
...
def hyperboloidGradient(x, s1, s2, rangediff):
    ...
    g = (
        2
        * (rho2 - rho1 - rangediff)
        * (rangeOfArrivalGradient(x, s2) - rangeOfArrivalGradient(x, s1))
    )
    return g


# @njit(nogil=True)
def hyperbolaGradDesc(
    pt,
    s1,
    s2,
    rangediff,
    step,
    epsilon,
    surfaceNorm=np.array([0, 0, 1], dtype=np.float64),
    verb=False,
):
    ...
    g = hyperboloidGradient(
        pt, s1, s2, rangediff
    )  # Calculate gradient at the point, use as a line
...
def gridSearchBlindLinearRTT(
    ...
    A = np.hstack((time_list.reshape((-1, 1)), np.ones((time_list.size, 1))))
...
def gridSearchRTT(
    ...
    for i in range(len(toa_list)):
        ...
        e_dist = np.linalg.norm(t - grid_list, axis=1) + np.linalg.norm(
            r - grid_list, axis=1
        )
...
def gridSearchTDOA(
    s1x_list, s2x_list, tdoa_list, td_sigma_list, xrange, yrange, z, verb=True
):
    """
    ...
    fullmesh = (
        np.vstack((xm.flatten(), ym.flatten(), np.zeros(len(ym.flatten())) + z))
        .transpose()
        .astype(np.float32)
    )
    cost_grid = 0
    ...
    for i in range(len(tdoa_list)):
        ...
        rm = np.linalg.norm(s2x - fullmesh, axis=1) - np.linalg.norm(
            s1x - fullmesh, axis=1
        )
...
def gridSearchFDOA(
    ...
    for i, fdoa in enumerate(nfdoa_list):
        ...
        parvm2 = np.dot(
            dirvecm2, s2v
        )  # This should already be negative when direction and velocities are opposed
        # print(parvm1)
        # print(parvm2)
...
def gridSearchTDOA_direct(
    s1x_list, s2x_list, tdoa_list, td_sigma_list, gridmat, verb=True
):
    """
    ...
    for i in range(len(tdoa_list)):
        ...
        rm = np.linalg.norm(s2x - gridmat, axis=1) - np.linalg.norm(
            s1x - gridmat, axis=1
        )
...
def calcCRB_TD(x, S, sig_r, pairs=None, cmat=None):
    ...
    if x.ndim == 1:
        x = x.reshape(
            (-1, 1)
        )  # Reshapes do not alter the external array (the one passed in)
...
def projectCRBtoEllipse(crb, pos, percent, dof=2, theta=None):
    ...
    r = (
        ...
        * a
        * b
        / np.sqrt(b**2 * np.cos(theta) ** 2 + a**2 * np.sin(theta) ** 2)
    )
...
class GridLocalizer:
    ...
    def plot(self, cost_grid: np.ndarray):
        ...
        """
        ax, img = pgPlotHeatmap(
            np.exp(
                -0.5 * cost_grid.reshape((self.yrange.size, self.xrange.size))
            ).T,  # must transpose
            self.xrange[0],
            self.yrange[0],
            np.ptp(self.xrange),
...
class LatLonGridLocalizer(GridLocalizer):
    ...
    def fromLatLonLimits(cls, centrelat, centrelon, latspan, lonspan, numLat, numLon):
        ecefgrid, lonlist, latlist = latlongrid_to_ecef(
            centrelat, centrelon, latspan, lonspan, numLat, numLon
        )
...
class TDMixin:
    def run(
        ...
        cost_grid = gridSearchTDOA_direct(
            s1x_list, s2x_list, tdoa_list, td_sigma_list, self.gridmat
        )
        return cost_grid
...
class TDFDMixin:
    def run(
        ...
        cost_grid = gridSearchTDFD_direct(
            s1x_list,
            s2x_list,
            tdoa_list,
            td_sigma_list,
...
def removeDownlinkRangeDiff(
    rx: np.ndarray, s1x: np.ndarray, s2x: np.ndarray, rangediff: np.ndarray
):
...
class SatellitePairTDFDMixin(TDFDMixin):
    ...
    def _generateSatellitesPosVel(self, times: np.ndarray):
        ...
        prigeocentrics = [
            sf_propagate_satellite_to_gpstime(self.prisat, time) for i in times
        ]
        secgeocentrics = [
            sf_propagate_satellite_to_gpstime(self.secsat, time) for i in times
        ]
        ...
        for i in range(len(times)):
            pri_x, pri_xdot = sf_geocentric_to_itrs(
                prigeocentrics[i], returnVelocity=True
            )
            sec_x, sec_xdot = sf_geocentric_to_itrs(
                secgeocentrics[i], returnVelocity=True
            )
    ...
    def run(
        self, times, tdoa_list, td_sigma_list, fdoa_list, fd_sigma_list, fc_up, fc_down
    ):
        """
        Performs TDOA+FDOA weighted least squares error calculations on every point in the grid.
        TDOAs are assumed to be measured as (time to sensor 2) - (time to sensor 1).
        ...
        pri_x_list, pri_xdot_list, sec_x_list, sec_xdot_list = (
            self._generateSatellitesPosVel(times)
        )
...
try:
    ...
    gridsearchtdoa_kernel = cp.RawKernel(
        r"""
    extern "C" __global__
