...
// void *threaded_wola(void *pArgs){
    ...
    int k;
	int tone_idx;
	Ipp64fc *y_chan; // holder pointer to the current channel
	...
	for (chanIdx = t_ID; chanIdx<numChans; chanIdx=chanIdx+NUM_THREADS){
		...
		for (nprime = 0; nprime<nprimePts; nprime++){
			...
			ippsDFTInv_CToC_64fc(dft_in, dft_out, pDFTSpec, pDFTBuffer);

			// === new code for general bin overlaps ===
			tone_idx = nprime % (N/Dec);
			ippsMul_64fc(dft_out, &tones[tone_idx * N], (Ipp64fc*)&out[nprimePts*chanIdx*N + nprime*N], N);
		}
	}
...
extern DLL_EXPORT int multiChanWOLA(Ipp64fc *y, double *f_tap, int fftlen, int Dec, int nprimePts, int L, int numChans, int chanLen, Ipp64fc *out){
	ippInit();
	Ipp64fc *tones;
	Ipp64f phase = 0;
	Ipp64f rFreq = 0;
