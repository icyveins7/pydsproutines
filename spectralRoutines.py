...
class IntegerMultipleFFT:
    def __init__(
        self, dtype=np.complex128, multiple: int = None, unpadLength: int = None
    ):
        """
    ...
    def computeInternals(self):
        ...
        self._tones = np.array(
            [
                np.exp(
                    -1j * 2 * np.pi * i / self._multiple * np.arange(self._N) / self._N
                )
    ...
    def fft(self, x: np.ndarray, reorder: bool = False):
        ...
        if x.size != self._N:
            raise Exception(
                "Input length does not correspond to internal value. Please call setUnpadLength()."
            )
...
class CZTCached:
    def __init__(self, xlength, f1, f2, binWidth, fs, convertTo32fc=False):
        ...
        while not foundGoodPrimes:
            if (
                np.max(sympy.primefactors(self.nfft)) <= 7
            ):  # change depending on highest tolerable radix
                foundGoodPrimes = True
                break
    ...
    def run(self, x):
        ...
        g = (
            g[self.m - 1 : self.m + self.k - 1]
            * self.ww[self.m - 1 : self.m + self.k - 1]
        )
    ...
    def runMany(self, xmany: np.ndarray, out: np.ndarray = None):
        ...
        fy = sp.fft.fft(
            y, self.nfft, axis=-1
        )  # actually it already does this by default
...
try:
    ...
    class CZTCachedGPU:
        def __init__(self, xlength, f1, f2, binWidth, fs):
            ...
            self.k = int(
                (f2 - f1) / binWidth + 1
            )  # This is the number of frequency bins
            self.m = xlength
            ...
            kk2 = kk**2.0 / 2.0
            self.d_ww = cp.exp(
                -1j * 2 * cp.pi * (f2 - f1 + binWidth) / (self.k * fs) * kk2
            )  # Compute in fc128, convert after for better precision
            chirpfilter = 1 / self.d_ww[: self.k - 1 + self.m]
            self.d_fv = cp.fft.fft(chirpfilter, self.nfft)

            nn = cp.arange(self.m)
            self.d_aa = (
                cp.exp(1j * 2 * cp.pi * f1 / fs * -nn) * self.d_ww[self.m + nn - 1]
            )
        ...
        def run(self, x: cp.ndarray):
            ...
            g = (
                g[self.m - 1 : self.m + self.k - 1]
                * self.d_ww[self.m - 1 : self.m + self.k - 1]
            )
    ...
    def toneSpectrum_gpu(f0, d_freqs, fs, N, phi=0.0, A=1.0):
        ...
        toneSpecKernel(
            (NUM_BLOCKS,),
            (THREADS_PER_BLOCK,),
            (phi, f0, d_freqs, d_freqs.size, np.float64(fs), int(N), A, out),
        )

        return out

    toneSpecMultiKernel = cp.RawKernel(
        r"""
    #include <cupy/complex.cuh>
    #include <math_constants.h>
    extern "C" __global__
    ...
    def toneSpectrumMulti_gpu(d_f0List, d_freqs, fs, N, d_phiList, d_AList):
        ...
        THREADS_PER_BLOCK = 256
        NUM_BLOCKS = int(out.size / THREADS_PER_BLOCK) + int(
            (out.size % THREADS_PER_BLOCK) > 0
        )

        if d_f0List.size > THREADS_PER_BLOCK:
            raise Exception(
                "Reduce parameter size. %d > %d" % (d_f0List.size > THREADS_PER_BLOCK)
            )
    ...
    def cupyDotTonesScaling(f0: float, fstep: float, numFreqs: int, src: cp.ndarray):
        ...
        NUM_BLOCKS = (
            NUM_BLOCKS + 1 if length % THREADS_PER_BLOCK > 0 else NUM_BLOCKS
        )  # increment if there's remainder
...
def toneSpectrum(f0, freqs, fs, N, phi=0, A=1.0):
    ...
    vals = (
        -1j
        * A
        * (1 - np.exp(-1j * 2 * np.pi * (freqs - f0) * N / fs))
        / (2 * np.pi * (freqs - f0) / fs)
        * np.exp(1j * phi)
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        fineFreqStep = 0.001
        freqs = np.arange(
            -fineFreqRange, fineFreqRange + fineFreqStep / 2, fineFreqStep
        )
        N = 145152
