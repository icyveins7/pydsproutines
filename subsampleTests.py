...
noise, rx = addSigToNoise(fs + 10, 0, sig, 100, 100, np.inf)
...
fig2.plot(np.abs(rx2), symbol='x', symbolPen='r')

# propagate by subsample for 1 sample
rx3, tone = propagateSignal(rx, T*1, fs, freq=10)
fig3 = pg.plot(title='full sample propagated')
fig3.plot(np.abs(rx3), symbol='x', symbolPen='r')

# check the tone is doing what it says
wrongslice = rx3[1:1+len(sig)]
wrongqf2 = np.abs(np.vdot(wrongslice, sig))**2 / np.linalg.norm(wrongslice)**2 / np.linalg.norm(sig)**2

rxbackshift = rx3 * tone.conj()
rxslice = rxbackshift[1:1+len(sig)]
qf2 = np.abs(np.vdot(rxslice, sig))**2 / np.linalg.norm(rxslice)**2 / np.linalg.norm(sig)**2