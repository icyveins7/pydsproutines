...
def simpleBinRead(
    filename, numSamps=-1, in_dtype=np.int16, out_dtype=np.complex64, offset=0
):
    """
    ...
    if in_dtype == np.complex64 or in_dtype == np.complex128:
        raise TypeError(
            "in_dtype must be a real type. You likely want float32 or float64 instead."
        )

    data = (
        np.fromfile(filename, dtype=in_dtype, count=numSamps * 2, offset=offset)
        .astype(np.float32)
        .view(out_dtype)
    )
...
def multiBinRead(
    filenames, numSamps, in_dtype=np.int16, out_dtype=np.complex64, offset=0
):
    """
    ...
    for i in range(len(filenames)):
        ...
        alldata[i * numSamps : (i + 1) * numSamps] = simpleBinRead(
            filename, numSamps, in_dtype, out_dtype, offset=offset
        )

    return alldata


def multiBinReadThreaded(
    filenames, numSamps, in_dtype=np.int16, out_dtype=np.complex64, offset=0, threads=2
):
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for future in concurrent.futures.as_completed(future_load):
            ...
            alldata[i * numSamps : (i + 1) * numSamps] = (
                future.result()
            )  # write to the mass array
...
class FolderReader:
    def __init__(
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(
            1
        )  # Generally good to have only 1 thread on concurrency for reading
        self.futures = []
    ...
    def refreshFilelists(self):
        ...
        reqMinFilesize = (
            baseObj.itemsize * 2 * self.numSampsPerFile
            if np.isrealobj(baseObj)
            else baseObj.itemsize * self.numSampsPerFile
        )
        ...
        if self.ignoreInsufficientData:
            dircontents = [
                ...
                if os.path.getsize(os.path.join(self.folderpath, i)) >= reqMinFilesize
            ]
    ...
    def get(self, numFiles: int, prefetch: int = 0):
        ...
        while len(self.futures) > 0 and remainderToRead > 0:
            ...
            data[i, :] = (
                future.result().astype(np.float32).view(self.out_dtype)
        ...
        for a in range(additional):
            if self.fidx + a < len(
                self.filepaths
            ):  # We only prefetch if there's files left
                self.futures.append(
                    futureBinRead(
                        self.executor,
                        self.filepaths[self.fidx + a],
                        self.numSampsPerFile,
                        self.in_dtype,
                    )
                )
    ...
    def getNextFile(self):
        ...
        alldata = simpleBinRead(
            fps, self.numSampsPerFile, self.in_dtype, self.out_dtype
        )
        return alldata, fps
    ...
    def _getbounds(self, numFiles, start):
        ...
        if numFiles is None:
            ...
            if numFiles * self.numSampsPerFile * self.sizeof > self.maxsizeof:
                raise MemoryError(
                    "Memory requested exceeds internal limit of %d bytes (self.maxsizeof). Please modify this at your own discretion."
                    % (self.maxsizeof)
                )
    ...
    def fastCheck(
        ...
        print(
            "Max/min real: %d,%d\nMax/min imag: %d,%d"
            % (int(maxreal), int(minreal), int(maximag), int(minimag))
        )
        ...
        if plots:
            ...
            ax[0].plot(
                np.arange(alldata.size)[::viewskip] / fs, np.abs(alldata)[::viewskip]
            )
            ax[1].plot(
                makeFreq(alldata.size, fs)[::viewskip],
                20 * np.log10(np.abs(np.fft.fft(alldata)))[::viewskip],
            )
...
class SortedFolderReader(FolderReader):
    ...
    def getFileByTime(self, reqTime):
        if isinstance(reqTime, int):
            ...
            alldata = multiBinReadThreaded(
                [path], self.numSampsPerFile, self.in_dtype, self.out_dtype
            )
            return alldata, path
        else:  # if array-like
            paths = [self.getPathByTime(i) for i in reqTime]
            alldata = multiBinReadThreaded(
                paths, self.numSampsPerFile, self.in_dtype, self.out_dtype
            )
    ...
    def splitHighAmpSubfolders(
        self,
        targetfolderpath: str,
        selectTimes: list = None,
        minAmp: float = 1e3,
        bufFront: int = 1,
        bufBack: int = 1,
        onlyExtractTimes: bool = False,
        onlyExtractGroups: bool = False,
        fmt: str = "%06d",
        ...
        if selectTimes is None:
            ...
            for i in range(len(self.filepaths)):
                ...
                if maxamp > minAmp:
                    ...
                    selectTimes.extend(
                        range(
                            self.filetimes[i] - bufFront,
                            self.filetimes[i] + bufBack + 1,
                        )
                    )
        else:
        ...
        if len(selectTimes) == 0:
            raise IndexError(
                "No groups were found. Perhaps try lowering the minAmp threshold?"
            )
        ...
        groupSplitIdx = np.hstack(
            (0, (np.argwhere(np.diff(selectTimes) > 1) + 1).flatten(), len(selectTimes))
        )
        if onlyExtractGroups:  # This will just return as a list of lists
            return [
                selectTimes[groupSplitIdx[i] : groupSplitIdx[i + 1]]
                for i in np.arange(groupSplitIdx.size - 1)
            ]
        ...
        for i in range(len(groupSplitIdx) - 1):
            ...
            if not useDatabase:
                ...
                srcfilepaths = [
                    os.path.join(self.folderpath, "%d.bin" % (i)) for i in grptimes
                ]
                dstfilepaths = [
                    os.path.join(subdirpath, "%d.bin" % (i)) for i in grptimes
                ]
                for p in range(len(srcfilepaths)):
                    print(
                        "Group %d: copy %s to %s"
                        % (i, srcfilepaths[p], dstfilepaths[p])
                    )
                    try:
                        shutil.copyfile(srcfilepaths[p], dstfilepaths[p])
                    except:
                        print(
                            "Error occurred while copying %s to %s"
                            % (srcfilepaths[p], dstfilepaths[p])
                        )
        ...
        if useDatabase:
            gd.updateMetatable(
                self.getFinalTime()
            )  # Update to the latest available time
...
class GroupDatabase:
    ...
    def addTable(self, tablename: str):
        stmt = (
            "create table if not exists %s(gidx INTEGER UNIQUE, starttime INTEGER, endtime INTEGER)"
            % tablename
        )
...
class LiveReader(FolderReader):
    ...
    def __init__(
        ...
        self.expectedFileSize = (
            numSampsPerFile * np.dtype(in_dtype).itemsize * 2
        )  # x2 for complex
    ...
    def getNext(self):
        ...
        if os.path.isfile(fp) and os.path.getsize(fp) >= self.expectedFileSize:
            # Read the file
            alldata = simpleBinRead(
                fp, self.numSampsPerFile, self.in_dtype, self.out_dtype
            )
            ...
            if self.exhaustFolderpath is not None:
                exhaustpath = os.path.join(
                    self.exhaustFolderpath, "%d%s" % (self.ftnow, self.extension)
                )
...
class SyncReaders:
    ...
    def fromSubdirs(
        ...
        subdirs = [
            os.path.join(topfolderpath, i)
            for i in os.listdir(topfolderpath)
            if os.path.isdir(os.path.join(topfolderpath, i))
        ]
        return cls(
            subdirs, numSampsPerFile, extension, in_dtype, out_dtype, ensure_incremental
        )
...
class FolderedGroupReader:
    ...
    def nextGroup(self):
        self.cGrp += 1
        reader = SortedFolderReader(
            os.path.join(self.folderpath, self.groups[self.cGrp]),
            self.numSampsPerFile,
            self.extension,
            self.in_dtype,
            self.out_dtype,
...
class GroupReaders:
    def __init__(
        ...
        self.groups0 = [
            i
            for i in os.listdir(folderpaths[0])
            if os.path.isdir(os.path.join(folderpaths[0], i))
        ]
        ...
        for i in range(1, len(folderpaths)):
            groupsi = [
                k
                for k in os.listdir(folderpaths[i])
                if os.path.isdir(os.path.join(folderpaths[i], k))
            ]
            seti = set(groupsi)
    ...
    def nextGroup(self) -> SyncReaders:
        ...
        self.groupdirpaths = [
            os.path.join(p, self.groups0[self.cGrp]) for p in self.folderpaths
        ]
...
class GroupReadersDB:
    def __init__(
        ...
        if not isinstance(folderpaths, list):
            raise TypeError(
                "folderpaths must be a list, even if only one folder is being used!"
            )
    ...
    def nextGroup(self, ensure_exists: bool = True):
        ...
        self.cStart, self.cEnd = self.grpdb.getGroupByIdx(
            "groups", self.cGrp
        )  # standard table name
        self.cFiles = [
            [
                os.path.join(folder, "%d" % (i) + self.extension)
                for i in range(self.cStart, self.cEnd + 1)
            ]
