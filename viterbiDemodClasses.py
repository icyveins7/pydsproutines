...
class ViterbiDemodulator:
    ...
    def run(self, y, pathlen):
        ...
        for a in np.arange(self.alphabetlen):
            ...
            for i in np.arange(self.L):
                ...
                xcs = (
                    np.exp(
                        ...
                        * (
                            -self.omegas[i]
                            * np.arange(0 * self.up, 0 * self.up + self.pulselen)
                        )
                    )
                    * xc
                )
            ...
            pathmetrics[a] = (
                np.linalg.norm(y[0 * self.up : 1 * self.up] - summed[: self.up]) ** 2
            )
        ...
        for n in np.arange(1, pathlen):
            ...
            branchmetrics, shortbranchmetrics = self.calcAllBranchMetrics(
                y, paths, pathmetrics, n
            )
            ...
            self.calcPathMetrics(
                shortbranchmetrics, branchmetrics, paths, pathmetrics, n
            )
        ...
        for p in np.arange(paths.shape[0]):
            ...
            for t in np.arange(len(self.pretransitions[p])):
                ...
                for i in np.arange(self.L):
                    ...
                    upguesspad = np.pad(
                        upguess[s : n * self.up + 1], (0, self.pulselen - 1)
                    )  # pad zeros to pulselen-1
                    ...
                    xcs = (
                        self.omegavectors[i, n * self.up : n * self.up + self.pulselen]
                        * xc
                    )
                ...
                branchmetrics[p, t] = (
                    np.linalg.norm(
                        y[self.up * n : self.up * n + self.pulselen] - summed
                    )
                    ** 2
                )
                shortbranchmetrics[p, t] = (
                    np.linalg.norm(
                        y[self.up * n : self.up * (n + 1)] - summed[: self.up]
                    )
                    ** 2
                )
    ...
    def calcPathMetrics(self, shortbranchmetrics, branchmetrics, paths, pathmetrics, n):
        ...
        for p in np.arange(branchmetrics.shape[0]):
            ...
            self.temppaths[p, :] = paths[
                self.pretransitions[p, bestPrevIdx], :
            ]  # copy the whole path over
            self.temppaths[p, n] = self.alphabet[p]
            self.temppathmetrics[p] = (
                pathmetrics[self.pretransitions[p, bestPrevIdx]]
                + shortbranchmetrics[p, bestPrevIdx]
            )
...
class BurstyViterbiDemodulator(ViterbiDemodulator):
    ...
    def __init__(
        ...
        super().__init__(
            alphabet, pretransitions, pulses, omegas, up, allowedStartIdx
        )  # identical
        ...
        self.newBurstPretransitions = np.array(
            [
                (
                    np.arange(self.alphabetlen)
                    if j in self.allowedStartIdx
                    else np.zeros(self.alphabetlen) - 1
                )
                for j in range(self.alphabetlen)
            ],
            dtype=np.int32,
        )
    ...
    def run(self, y, pathlen):
        ...
        for n in np.arange(1, pathlen):
            ...
            if n % self.numPeriodSyms == 0:
                newburstBranchmetrics, newburstShortBranchMetrics = (
                    self.calcNewBurstBranchMetrics(y, paths, pathmetrics, n)
                )
                self.calcNewBurstPathMetrics(
                    newburstShortBranchMetrics,
                    newburstBranchmetrics,
                    paths,
                    pathmetrics,
                    n,
                )
    ...
    def calcNewBurstBranchMetrics(self, y, paths, pathmetrics, n):
        ...
        for p in self.allowedStartIdx:
            ...
            for t in np.arange(len(self.newBurstPretransitions[p])):
                print(
                    "Calculating for alphabet idx %d, from previous burst alphabet idx %d"
                    % (p, self.newBurstPretransitions[p, t])
                )
                ...
                x_all = np.zeros(
                    (self.L, guardlen + self.pulselen), dtype=np.complex128
                )

                # Convenience indexing for extraction with reference to original signal length
                extractionIdx = np.arange(
                    (n - self.numGuardSyms) * self.up, n * self.up + self.pulselen
                )
                shortextractionIdx = np.arange(
                    (n - self.numGuardSyms) * self.up, (n + 1) * self.up
                )
                ...
                for i in np.arange(self.L):
                    ...
                    xc = sps.lfilter(self.pulses[i], 1, upguesspad)[
                        -(self.pulselen + guardlen) :
                    ]
                ...
                shortbranchmetrics[p, t] = (
                    np.linalg.norm(y[shortextractionIdx] - summed[: guardlen + self.up])
                    ** 2
                )
    ...
    def calcNewBurstPathMetrics(
        self, newburstShortBranchMetrics, newburstBranchmetrics, paths, pathmetrics, n
    ):
        ...
        for p in np.arange(newburstBranchmetrics.shape[0]):
            ...
            self.temppaths[p, :] = paths[
                self.newBurstPretransitions[p, bestPrevIdx], :
            ]  # copy the whole path over
            self.temppaths[p, n] = self.alphabet[p]
            self.temppathmetrics[p] = (
                pathmetrics[self.newBurstPretransitions[p, bestPrevIdx]]
                + newburstShortBranchMetrics[p, bestPrevIdx]
            )

            print(
                "Selected (oldburst) %d->%d (newburst)"
                % (self.newBurstPretransitions[p, bestPrevIdx], p)
            )
