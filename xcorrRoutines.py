...
class GroupXcorrCZT:
    ...
    def xcorr(self, rx: np.ndarray, shifts: np.ndarray = None):
        ...
        groupPhases = np.exp(
            -1j * 2 * np.pi * cztFreq * self.starts.reshape((-1, 1)) / self.fs
        )
        ...
        for i in np.arange(shifts.size):
            ...
            for g in np.arange(self.numGroups):
                ...
                rxgroup = rx[
                    shift + self.starts[g] : shift + self.starts[g] + self.lengths[g]
                ]
...
try:
    ...
    class GroupXcorrFFT:
        def __init__(
            ...
            self.groupPhases = np.exp(
                -1j * 2 * np.pi * self.fftfreq * self.starts.reshape((-1, 1)) / self.fs
            )
        ...
        def _xcorrThread(self, rx: np.ndarray, shifts: np.ndarray):
            ...
            for i, shift in enumerate(shifts):
                ...
                for g in np.arange(self.numGroups):
                    ...
                    rxgroup = rx[
                        shift + self.starts[g] : shift + self.starts[g] + self.ygroupLen
                    ]
                    rxgroupNormSqCollect[g] = np.linalg.norm(rxgroup) ** 2

                    # pdt[g,:self.ygroupLen] = ygroup * rxgroup
                    np.multiply(
                        ygroup, rxgroup, out=pdt[g, : self.ygroupLen]
                    )  # ufunc direct
                ...
                xc_slice[i, :] = (
                    np.abs(pdtfftCombined) ** 2 / rxgroupNormSq / self.ygroupNormSq
                )

            return xc_slice

        def xcorrThreads(
            self, rx: np.ndarray, shifts: np.ndarray = None, NUM_THREADS: int = 4
        ):
            if shifts is None:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = {
                    executor.submit(self._xcorrThread, rx, shifts[i::NUM_THREADS]): i
        ...
        def xcorr(
            self, rx: np.ndarray, shifts: np.ndarray = None, flattenToTime: bool = True
        ):
            if shifts is None:
                shifts = np.arange(len(rx) - (self.starts[-1] + self.fftlen) + 1)
            else:
        ...
        def xcorrGPU(
            self, rx: cp.ndarray, shifts: np.ndarray = None, flattenToTime: bool = True
        ):
            if shifts is None:
                shifts = np.arange(len(rx) - (self.starts[-1] + self.fftlen) + 1)
            else:
            ...
            yStarts = (
                cp.arange(self.numGroups, dtype=cp.int32) * self.fftlen
            )  # Constant
            ...
            for i, shift in enumerate(shifts):
                ...
                if flattenToTime:
                    ...
                    xc[i, :] = (
                        cp.abs(pdtfftCombined) ** 2 / rxgroupNormSq / self.ygroupNormSq
                    )
    ...
    class GroupXcorrCZT_Permutations:
        ...
        def __init__(
            self,
            ygroups: np.ndarray,
            ygroupIdxs: np.ndarray,
            groupStarts: np.ndarray,
            f1: float,
            f2: float,
            binWidth: float,
            fs: int,
            ...
            assert (
                ygroups.shape[0] == ygroupIdxs.size
            )  # ensure numTemplates corresponds
            assert (
                np.unique(ygroupIdxs).size == groupStarts.size
            )  # ensure numGroups corresponds
            ...
            assert np.all(
                np.sort(np.unique(ygroupIdxs)) == np.arange(self.numGroups)
            )  # ensure all groups accounted for and was sorted
        ...
        def xcorrGPU(self, rx: cp.ndarray, shifts: np.ndarray, batchSz=32):
            ...
            self.d_xcTemplates = cp.zeros(
                (
                    self.numTemplates,
                    shifts.size,
                    int((self.f2 - self.f1) / self.binWidth + 1),
                ),
                dtype=cp.complex64,
            )
            self.d_rxgroupNormSq = cp.zeros(
                (self.numGroups, shifts.size), dtype=cp.float32
            )

            # Pre-calculate the phases for each group (TODO: i should move to this init?)
            cztFreq = cp.arange(
                self.f1, self.f2 + self.binWidth / 2, self.binWidth
            )  # Compute as doubles
            groupPhases = cp.exp(
                -1j
                * 2
                * cp.pi
                * cztFreq
                * cp.array(self.groupStarts.reshape((-1, 1)))
                / self.fs
            ).astype(
                cp.complex64
            )  # Convert to floats
            ...
            numPerBatch = np.hstack(
                (
                    np.zeros(int(shifts.size / batchSz), dtype=np.int32) + batchSz,
                    np.remainder(shifts.size, batchSz, dtype=np.int32),
                )
            )
            ...
            for k in np.arange(self.numTemplates):
                ...
                if groupNormSqCompleted[groupNumber] == 0:
                    ...
                    for i in np.arange(shifts.size):
                        self.d_rxgroupNormSq[groupNumber, i] = cp.sum(
                            d_rxSliceAbsSq[i : i + self.length]
                        )
                ...
                for b in np.arange(numPerBatch.size):
                    ...
                    cztcg.runMany(
                        d_mulMatrix[: numPerBatch[b]],
                        out=d_pdtcztMatrix[: numPerBatch[b]],
                    )
                    ...
                    cp.multiply(
                        d_pdtcztMatrix[: numPerBatch[b]],
                        groupPhases[groupNumber, :],
                        out=self.d_xcTemplates[
                            k,
                            startIdxPerBatch[b] : startIdxPerBatch[b] + numPerBatch[b],
                            :,
                        ],
                    )  # Write to memory directly
        ...
        def getCAF_GPU(self, templateIdx: np.ndarray):
            ...
            # Initialize array
            cafcplx = cp.zeros(
                (self.d_xcTemplates.shape[1], self.d_xcTemplates.shape[2]),
                dtype=np.complex64,
            )
            ...
            for groupNumber in np.arange(templateIdx.size):
                templateNumber = np.argwhere(self.ygroupIdxs == groupNumber)[
                    templateIdx[groupNumber]
                ][
                    0
                ]  # cupy needs the 0
        ...
        def xcorr(self, rx: np.ndarray, shifts: np.ndarray = None, numThreads: int = 1):
            ...
            if shifts is None:
                ...
                assert (
                    shifts[-1] + self.groupStarts[-1] + self.length < rx.size
                )  # make sure it can access it

            # Init memory for outputs
            self.xcTemplates = np.zeros(
                (
                    self.numTemplates,
                    shifts.size,
                    int((self.f2 - self.f1) / self.binWidth + 1),
            ...
            cztFreq = np.arange(self.f1, self.f2 + self.binWidth / 2, self.binWidth)
            groupPhases = np.exp(
                -1j * 2 * np.pi * cztFreq * self.groupStarts.reshape((-1, 1)) / self.fs
            )
        ...
        def getCAF(self, templateIdx: np.ndarray, numThreads: int = 4):
            ...
            # Initialize array
            cafcplx = np.zeros(
                (self.xcTemplates.shape[1], self.xcTemplates.shape[2]),
                dtype=np.complex128,
            )
        ...
        def _getCAFThread(
            ...
            for groupNumber in np.arange(templateIdx.size):
                ...
                if thIdx == numThreads - 1:
                    ...
                    cafcplx[thIdx * numPerThread : (thIdx + 1) * numPerThread, :] = (
                        cafcplx[thIdx * numPerThread : (thIdx + 1) * numPerThread, :]
                        + self.xcTemplates[
                            templateNumber,
                            thIdx * numPerThread : (thIdx + 1) * numPerThread,
                            :,
                        ]
                    )
        ...
        def _xcorrThread(
            ...
            fs,
            xcTemplates,
            rxgroupNormSq,  # outputs, written directly to array
            numThreads,
            thIdx,
        ):
            ...
            for i in np.arange(thIdx, shifts.size, numThreads):
                ...
                for k in np.arange(numTemplates):
                    groupNumber = ygroupIdxs[
                        k
                    ]  # Get the group number for this template
                    ygroup = ygroups[k, :]  # Get the array for this template
                    # Slice the correct start index for this group
                    rxgroup = rx[
                        shift
                        + groupStarts[groupNumber] : shift
                        + groupStarts[groupNumber]
                        + length
                    ]
                    if (
                        rxgroupNormSq[groupNumber, i] == 0
                    ):  # Write to the normSq if not yet done i.e. only first template of the group
                        rxgroupNormSq[groupNumber, i] = (
                            np.linalg.norm(rxgroup) ** 2
                        )  # may want to wrap this in if-else since re-calculated
    ...
    # %%
    group_xcorr_kernel = cp.RawKernel(
        r"""
    ...
    group_xcorr_kernelv2 = cp.RawKernel(
        r"""
    #include <cupy/complex.cuh>
    extern "C" __global__
    ...
    class GroupXcorrGPU(GroupXcorr):
        ...
        def xcorr(self, rx: np.ndarray, shifts: np.ndarray = None):
            ...
            for i in np.arange(shifts.size):
                ...
                # Save output (with normalisations)
                d_xc[i] = (
                    d_pfabs[d_pmaxind] ** 2 / d_rxconcatNormSq / self.d_yconcatNormSq
                )
        ...
        def xcorrKernel(self, rx, shifts, numShiftsPerBlk=2, verbTiming=False):
            ...
            gIdx = np.hstack(
                [
                    np.arange(self.starts[i], self.starts[i] + self.lengths[i])
                    for i in range(self.numGroups)
                ]
            )
            ...
            group_xcorr_kernelv2(
                ...
                (
                    ...
                    self.d_yconcat,
                    ylen,
                    self.yconcatNormSq.astype(np.float32),
                    self.d_freqMat,
                    numFreqs,
                    d_gIdx,
                    d_shifts,
...
def computeFastXcorrComplexity(N: int, K: int = 1):
    """
...
def computeGroupXcorrCZTcomplexity(m: int, L: int, n: int, K: int = 1):
    """
